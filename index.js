  ...
  var teks = text.replace(/!nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '[ WAIT ] Sedang Menulis❗', MessageType.text)
...
axios.get(`http://scrap.terhambar.com/yt?link=${teks}`).then((res) => {
    ...
    let hasil = `Link : *${teks}*✅Lagu Berhasil Di Download, silahkan klik link dan download hasilnya\nKlik link dibawahn\nJudul: ${res.data.title}\n\nDuration: ${res.data.inText}\n\nAudio: ${res.data.linkAudioOnly}`;
...
axios.get(`https://st4rz.herokuapp.com/api/chord?q=${teks}`).then((res) => {
	conn.sendMessage(id, '[ WAIT ] Sedang mencari chord lagu *${teks}*❗', MessageType.text)
...
axios.get(`http://scrap.terhambar.com/tiktokfull?link=${tictoc}`).then((res) => {
	 conn.sendMessage(id, '[ WAIT ] Sedanv Diproses❗', MessageType.text)
...
if (text.includes("!sholat")){
  ...
  conn.sendMessage(id, '[ WAIT ] Sedang Mencari Jadwal Di Daerah *${teks}*❗', MessageType.text)
...
var tampilWaktu = "JAM: " + jam + ":" + menit + ":" + detik;
conn.sendMessage(id, donate.donate(id, BotName, corohelp, tampilTanggal, tampilWaktu, instagramlu, whatsapplu, kapanbotaktif, grupch1, grupch2) ,MessageType.text);
conn.sendMessage(id, 'Oke Gan Jangan Lupa Donasinya😁\n\n*_Powered By FarhanXCode7_*', MessageType.text);
}
else if (text == '#DONATE'){
const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
...
conn.sendMessage(id, 'Jangan Lupa ketik *#donasi* untuk berdonasi😁', MessageType.text);
}
else if (text == '#readme'){
  const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
...
conn.sendMessage(id, 'Jangan Lupa ketik *#donasi* Untuk Berdonasi Gan😁', MessageType.text);
}
else if (text == '!foto'){
...
else if (caption == '!stiker'){
conn.sendMessage(id, '❗Perintah Salah❗\n\n Yg Bener kirim foto dengan Capt *!sticker*', MedsageType.caption);
}
...
┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
    ...
    `, MessageType.text);
 conn.sendMessage(id, '\nBuset Cocok Dah Gan👍', MessageType.text);
  });
  }
   if (text.includes("!foto cewek"))
   {
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
    conn.sendMessage(id, '[ WAIT ] Sedang mencari foto janda❗', MessageType.text)
	var buf = Buffer.from(response, 'base64'); // Ta-da	
              conn.sendMessage(
            id,
              buf,MessageType.image)
   ...
   if (text.includes("!foto cowok"))
   {
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
  conn.sendMessage(id, '[ WAIT ] Sedang mencari foto cogan❗', MessageType.text)
  var buf = Buffer.from(response, 'base64'); 
              conn.sendMessage(
            id,
              buf,MessageType.image)
...
if (text.includes("!fotoanime"))
   {
    ...
    axios.get(url)
      .then((result) => {
        ...
        .then(
            (response) => {
    conn.sendMessage(id, '[ WAIT ] Sedang mencari foto anime❗', MessageType.text)
	var buf = Buffer.from(response, 'base64'); 
              conn.sendMessage(
            id,
              buf,MessageType.image)
...
if (text.includes("!lirik")){
	...
	axios.get(`http://scrap.terhambar.com/lirik?word=${teks}`).then ((res) => {
	     conn.sendMessage(id, '[ WAIT ] Search lirik yg kamu minta❗', MessageType.text)
